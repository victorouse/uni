<?php
    ...
    class DataModel {
        ...
        public function getIssueComments($issueID) {
            $query = "SELECT *, (upvotes - downvotes) AS votes FROM IssueComments INNER JOIN Comments
                ON IssueComments.comment_id = Comments.comment_id
        ...
        public function addCommentReply($commentID, $title, $comment, $author) {
            ...
            $query = "INSERT INTO CommentReplies VALUES (?, ?)";
            $stmt = $this->db->prepare($query);
        ...
        public function getReplies($commentID) {
            $query = "SELECT * FROM Comments WHERE Comments.comment_id IN 
                (SELECT reply_id FROM Comments JOIN CommentReplies 
                ON Comments.comment_id = CommentReplies.comment_id 
                WHERE CommentReplies.comment_id = ?) ORDER BY create_dt DESC";
            $stmt = $this->db->prepare($query);
            $stmt->execute(array($commentID));
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        public function checkVoted($memberID, $id, $type) {
            if ($type == "issue") {
                $query = "SELECT EXISTS (SELECT * FROM MemberIssueVotes WHERE member_id = ? AND issue_id = ?)";
            } else if ($type == "comment") {
                $query = "SELECT EXISTS (SELECT * FROM MemberCommentVotes WHERE member_id = ? AND comment_id = ?)";
            }
        ...
        public function changeVote($memberID, $id, $action, $type) {
            if ($action == "up") {
                if ($type == "issue") {
                    $query = "UPDATE MemberIssueVotes SET voted_up = 1, voted_down = 0
                        WHERE member_id = ? AND issue_id = ?";
                } else if ($type == "comment") {
                    $query = "UPDATE MemberCommentVotes SET voted_up = 1, voted_down = 0
                        WHERE member_id = ? AND comment_id = ?";
                }
            } else {
                if ($type == "issue") {
                    $query = "UPDATE MemberIssueVotes SET voted_up = 0, voted_down = 1
                        WHERE member_id = ? AND issue_id = ?";
                } else if ($type == "comment") {
                    $query = "UPDATE MemberCommentVotes SET voted_up = 0, voted_down = 1
                        WHERE member_id = ? AND comment_id = ?";
                }
            }
        ...
        public function getVote($memberID, $id, $type) {
            if ($type == "issue") {
                $query = "SELECT * FROM MemberIssueVotes WHERE member_id = ? AND issue_id = ?";
            } else if ($type == "comment") {
                $query = "SELECT * FROM MemberCommentVotes WHERE member_id = ? AND comment_id = ?";
            }
        ...
        public function addVote($memberID, $id, $action, $type) {
            if ($action == "up") {
                if ($type == "issue") {
                    $query = "INSERT INTO MemberIssueVotes VALUES (?, ?, 1, 0)";
                } else if ($type == "comment") {
                    $query = "INSERT INTO MemberCommentVotes VALUES (?, ?, 1, 0)";
                }
            } else {
                if ($type == "issue") {
                    $query = "INSERT INTO MemberIssueVotes VALUES (?, ?, 0, 1)";
                } else if ($type == "comment") {
                    $query = "INSERT INTO MemberCommentVotes VALUES (?, ?, 0, 1)";
                }
            }
